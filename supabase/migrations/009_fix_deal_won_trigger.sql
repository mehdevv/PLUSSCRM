-- Fix deal-won trigger: invalid ON CONFLICT DO NOTHING (no unique constraint on clients)
-- caused 400 errors when marking deals as WON.
...
BEGIN
  IF NEW.stage = 'WON' AND (OLD.stage IS DISTINCT FROM 'WON') THEN
    ...
    ELSE
      ...
      VALUES (v_company, v_contact, v_email, l.phone, COALESCE(NEW.value, 0), 1, NEW.rep_id, l.country, 'Deal won');
    END IF;
