-- When a deal leaves Won, reverse payments, commissions, rep points, and client revenue
...
DECLARE
  ...
  client_id UUID;
BEGIN
  DELETE FROM payments WHERE deal_id = p_deal_id;
  ...
  FROM (
    ...
    JOIN leads dl ON dl.id = d.lead_id
    WHERE d.rep_id = p_rep_id
      AND d.stage = 'WON'
...
BEGIN
  IF OLD.stage = 'WON' AND NEW.stage IS DISTINCT FROM 'WON' THEN
    IF EXISTS (SELECT 1 FROM payments WHERE deal_id = NEW.id)
...
GRANT EXECUTE ON FUNCTION public.revert_deal_won_effects(UUID, UUID, UUID) TO authenticated, service_role;
