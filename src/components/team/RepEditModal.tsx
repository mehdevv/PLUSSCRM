...
interface RepEditModalProps {
  ...
  saving?: boolean;
  onClose: () => void;
  onSave: (updates: { name: string; initials: string; color: string; tier: RepTier; vacation_mode: boolean }) => void;
}

export function RepEditModal({ rep, open, saving, onClose, onSave }: RepEditModalProps) {
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-card border border-card-border rounded-xl shadow-lg w-full max-w-md p-6">
        ...
        <form
          onSubmit={(e) => {
            e.preventDefault();
            onSave({ name: name.trim(), initials: initials.trim(), color, tier, vacation_mode: vacationMode });
          }}
          className="space-y-3"
        >
